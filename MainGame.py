...
class MainGame:
    ...
    def __init__(self,game_speed, num_ghosts ):
        ...
        self.ghosts = init_ghosts(self,self.num_ghosts)
        ...
        def update_game(Self):
            ...
            if self.time_to_reset is not None and Self.cur_time_passed > self.time_to_reset:
                self.reset()
                self.time_to_reset = None
            self.pacman.movement_logic()
            for ghost in self.ghosts:
                ghost.movement_logic()
    ...
    def game_run(self):
        ...
        while self.running:
            ...
            self.draw_frame()
            self.check_collisions()
            self.clock.tick(self.game_speed)  # sets framerate
        pygame.quit()
    ...
    def get_game_state(self):
        return {
            'pacman':(self.pacman.x,self.pacman.y,self.pacman.cur_mode),
            'ghosts': [(ghost.x,ghost.y,ghost.cur_mode)for ghost in self.ghosts]
        }
    def set_pacman_direction(self, direction):
        self.pacman.keyboard_input(direction)
